...
*/
    ...
    this.getDataOK=function (data) {
		...
		//console.log("RSSImin="+data.rssimin);
...
*/
		...
		//console.log("LASTCMD="+data.lastCmd);
/*
		var c=data.lastcmd=='on'?'green':'red';
		$('#fan').css('background-color', c).css('color', c);
*/
        if (data.lastCmd =='on') {
            $('#Switch-ON').show();$('#Switch-OFF').hide();
        } else {
            $('#Switch-OFF').show();$('#Switch-ON').hide();
        }
    ...
    this.drawRssiMeter=function (data) {
		var rssi=Math.floor(data.rssi);
		var rssimin=Math.floor(data.rssimin);
		...
		//rssiMeterOptions.min=thiz.minmin;
		//rssiMeterOptions.max=thiz.maxmax;
		...
		//rssiMeterOptions.redFrom=Math.min(this.minmin,rssimin)-2*step;
		rssiMeterOptions.redFrom=rssiMeterOptions.min;
    ...
    this.setRssiVal=function() {
        var newThreshold =$('#rssid-val-text').val().trim();
        if (newThreshold < rssiMeterOptions.min) newThreshold=rssiMeterOptions.min;
        if (newThreshold > rssiMeterOptions.max) newThreshold=rssiMeterOptions.max;
		$.ajax({url: "/status",
			success: thiz.getDataOK,
			error: thiz.getDataErr,
			dataType: "json",
			method: 'POST',
			data:"rssimin="+newThreshold
			});
    };
