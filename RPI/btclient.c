...
void bt_message_callback(struct mosquitto *mqtt, void *udata, const struct mosquitto_message *message) {
	tprintf("****** MSG cb topic='%s' payload='%s'\n", message->topic, message->payload);
/*
...
/* send the action to the Raspberry SenseHAT */
		sts=execl("btclient.d/run-command.py", "run-command", (const char*)NULL);
		tprintf("something went wrong calling the python script, error='%s'\n", strerror(errno));
	break;
	case -1:	// error
...
void bt_subscribe_callback(struct mosquitto *mqtt, void *obj, int mid, int qos_count, const int *granted_qos) {
	tprintf("sub cb %d subscriptions\n", qos_count);
}
...
tprintf("Setting Deviceid=%s \n", cfg.deviceid);
	...
	tprintf("now setting username and password to %s/%s\n", cfg.username,cfg.password);
	...
	/* set the TLS required information */

	tprintf("now setting TLS required paramters\n");
	sts = mosquitto_tls_set(mqtt, "cert/messaging.pem", NULL, NULL, NULL, NULL);
	if(sts) {
		tprintf("Error setting TSL parameters. Err: %d\n",sts);
		return sts;
	}

	tprintf("now setting TLS optional  parameters\n");
	sts = mosquitto_tls_opts_set(mqtt, 1, "tlsv1.2", NULL);
	if(sts) {
		tprintf("Error setting TSL OPTS parameters. Err: %d\n",sts);
		return sts;
	}

	tprintf("now connection using host=%s port=%d\n", cfg.host, cfg.port);
/*
	sts = mosquitto_connect_bind(mqtt, cfg.host, cfg.port, 60, NULL);
*/
	sts = mosquitto_connect(mqtt, cfg.host, cfg.port, 60);
	...
	for (;is_connected != CONN_ERR;) {
		...
		else tprintf("waiting for connection to come up\n", "");
		sleep(cfg.sleep);
	}
	blue_disconnect();
